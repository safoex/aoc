...
class HierarchicalClassifier:
    ...
    def save_local_aae(self, global_class, trainer_params, batch_size=64, num_workers=8):
        ...
        self.train_aae(aae, ds, class_workdir + '/' + 'local%d.pth' % aae.latent_size, trainer_params, batch_size,
                       num_workers)
    ...
    def save_global_classifier(self, max_epochs=2, subset_fraction=0.3, extra_folder=""):
        ...
        if extra_folder and not os.path.exists(self.workdir + '/' + extra_folder):
            os.mkdir(self.workdir + '/' + extra_folder)
        torch.save(self.global_classifier.state_dict(), self.workdir + '/' + extra_folder + '/' + 'global.pth')

    def load_global_classifier(self, extra_folder=""):
        self.global_classifier = ObjectClassifier(len(self.classes)).to(self.device)
        self.global_classifier.load_state_dict(torch.load(self.workdir + '/' + extra_folder + '/' + 'global.pth'))
    ...
    def save_local_classifiers(self, max_epochs=3, threshold=0.2, extra_folder=""):
        ...
        for cl, subclasses in self.classes.items():
            ...
            grid_subsets = torch.stack(
                [torch.tensor([is_valid_rotation(rot, ilbler) for rot in self.dataset.grider.grid]) for ilbler in
                 range(2)])
            ...
            if extra_folder and not os.path.exists(self.workdir + '/' + extra_folder):
                os.mkdir(self.workdir + '/' + extra_folder)
            torch.save(self.in_class_classifiers[cl].state_dict(), self.workdir + '/' + extra_folder + '/' + '%s.pth' % cl)

    def load_local_classifiers(self, extra_folder=""):
        self.in_class_classifiers = {
            cl: ObjectClassifier(len(subclasses)).to(self.device) for cl, subclasses in self.classes.items()
        }
        for cl in self.classes:
            self.in_class_classifiers[cl].load_state_dict(torch.load(self.workdir + '/' + extra_folder + '/' + '%s.pth' % cl))
    ...
    def load_everything(self, extra_folder_global="", extra_folder_local=""):
        self.load_global_classifier(extra_folder=extra_folder_global)
        self.load_local_classifiers(extra_folder=extra_folder_local)
