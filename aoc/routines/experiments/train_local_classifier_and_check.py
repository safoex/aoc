...
for max_epochs in range(2, 4):
    for subset_fraction_int in range(2, 6):
        subset_fraction = subset_fraction_int / 10.0
        ioe.tc.hcl.save_global_classifier(extra_folder="global_%d_%1.f" % (max_epochs, subset_fraction),
                                          max_epochs=max_epochs, subset_fraction=subset_fraction)
